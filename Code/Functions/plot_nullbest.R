plot_nullbest <- function(outputs, .font_size = font_size, .pt_size = pt_size, N_iter, ind) {
  ...
  null_data <- as.data.frame(matrix(NA, nrow = 1, ncol = 3)) ##create empty dataframes to store data
  alliter_data <- as.data.frame(matrix(NA, nrow = 1, ncol = 3))
  colnames(null_data) <- c("pattern 4", "Method", "SampleSize")
  colnames(alliter_data) <- c("pattern 4", "Method", "SampleSize")
  ...
  null_data <- null_data[!is.na(null_data$`pattern 4`),] ##remove NAs 
  ...
  for(q in n){
    for (m in unique(null_data$Method)){
      ...
      plotting[1,1] <- sum(sample_meth[, 1] == 1) / (nrow(sample_meth)) ## sum of times Tx 1 = identified_best_t / # iterations per sample size and method 
      plotting[1,2] <- sum(sample_meth[, 1] == 2) / (nrow(sample_meth)) 
      plotting[1,3] <- sum(sample_meth[, 1] == 3) / (nrow(sample_meth))
      plotting[1,4] <- sum(sample_meth[, 1] == 4) / (nrow(sample_meth)) 
  ...
  plotm1 <- plotm1[!is.na(plotm1$`Treatment 4`),] ##remove NAs 
  
  
  plotm1 <- reshape2::melt(plotm1, id.var = c("Method", "Sample Size"))
  
  fm <- ggplot(plotm1, aes(x =`Sample Size`, y = value, fill = variable)) +
    geom_bar(position = "dodge", stat = "identity") + scale_x_continuous(breaks = c(1000, 2000, 3000, 4000, 5000)) + scale_y_continuous(breaks = c(0, 0.25, 0.5), limits = c(0, 0.5)) + facet_grid(. ~ Method)+ylab("Proportion of Bests") + xlab("Sample Size")+labs(fill = NULL)+theme_minimal() +
    theme(
