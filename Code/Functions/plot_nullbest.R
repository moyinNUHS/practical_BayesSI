...
plot_nullbest <- function(outputs, .font_size = font_size, .pt_size = pt_size, N_iter, ind) {
  ...
  for (i in n) {
    ...
    for(a in 1:N_iter){
      ...
      alliter_data <- rbind(alliter_data, subset_iter) 
  ...
  null_data <- null_data[!is.na(null_data$`pattern 1`),]
  ...
  for(q in n){
    for (m in unique(null_data$Method)){
     ...
     plotting[1,1] <- sum(sample_meth[, c(2,4)] == 1) / nrow(sample_meth) * 2
     plotting[1,2] <- sum(sample_meth[, 1:4] == 2) / nrow(sample_meth) * 4
     plotting[1,3] <- sum(sample_meth[, 1:4] == 3) / nrow(sample_meth) * 4
     plotting[1,4] <- sum(sample_meth[, 3:4] == 4) / nrow(sample_meth) * 2
     plotting[1,5] <- m
     plotting[1,6] <- q
  ...
  plotm1 <- plotm1[!is.na(plotm1$`Treatment 1`),]
