---
title: "Plots for simulation scenarios with 80 confidence interval for scenarios 1.1 to 1.5 and 4.2"
author: "Xinru WANG"
date: "2024-01-14"
...
files = list.files(path = paste0(wd, 'Code/Run_output'), full.names = TRUE, pattern = "iter1000_2024-01-13.rds")
...
scenario. = c("1.1","1.2","1.3","1.4","1.5","4.2")
...
```{r, echo = FALSE, fig.align="center",fig.height = 14, fig.width = 18}
# load data in the output folder 
files = list.files(path = paste0(wd, 'Code/Run_output'), full.names = TRUE, pattern = "_warnings.Rdata")

extract_info <- function(file) {
  parts <- strsplit(file, "_")[[1]]
  scenario <- sub(".*scenario", "scenario", parts[2])
  sample_size <- sub("size=", "", parts[5])
  return(list(scenario = scenario, sample_size = sample_size))
}

# Extract information and store in a list
info_list <- lapply(files, extract_info)
# Unique scenarios and sample sizes
unique_scenarios <- unique(sapply(info_list, function(x) x$scenario))
unique_sample_sizes <- unique(sapply(info_list, function(x) x$sample_size))

# Nested list structure
nested_list <- list()
for(scenario in unique_scenarios) {
  nested_list[[scenario]] <- list()
  for(sample_size in unique_sample_sizes) {
    # Filter files for each scenario and sample size
    scenario_files <- files[sapply(info_list, function(x) x$scenario == scenario & x$sample_size == sample_size)]
    load(scenario_files)
    if(!all(is.na(warning_data))){
      nested_list[[scenario]][[sample_size]] <- scenario_files
    }
  }
}
nested_list
