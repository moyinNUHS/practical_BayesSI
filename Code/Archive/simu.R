...
#wd <- "/Users/cheryl/Documents/duke-nus/bibhas/ordinalDTR/BayesianOrdinal/"
...
simulation <- function(N,      
  ...
  output_replication <- foreach(k = 1:R) %dopar% {
    ...
    wd = '/data/chakraborty/home/e0859927/practical/'
    setwd(wd)
    scripts = paste0(wd, 'Code/Functions/', list.files('Code/Functions/')[which(list.files('Code/Functions/')!="simulation.R" & list.files('Code/Functions/')!="simu.R")])
    lapply(scripts, source)
    ...
    est_method_2_str <-fit_model_2_prior(nma_data_prior, nma_data, Trial_Treat_lab_vec, Scale = Scale_str) # use current trial data
    #est_method_2_non <-fit_model_2_noprior(nma_data, Trial_Treat_lab_vec) # use current trial data
...
run_simulation(pattern_list = list( # Treatment patterns
  pattern1 = c(1, 2, 3, 4), 
  pattern2 = c(1, 2, 3, 4),
  pattern3 = c(1, 2, 3, 4),
  pattern4 = c(1, 2, 3, 4)),
  prob_pattern = c(P1 = 0.25, P2 = 0.25, P3 = 0.25, P4 = 0.25), # Prevalence of each pattern
               ...
               scenario_name = paste0(c('scenario1.1', paste0('iter', No_iter),"pattern_same"), collapse = '_')
)
...
run_simulation(
  ...
  samplesize_hist = N_hist, #Sample size of historical trial
  N_iter = No_iter,          # Number of iterations
  scenario_name = paste0(c('scenario1.1', paste0('iter', No_iter)), collapse = '_')
)
...
run_simulation(pattern_list = list( # Treatment patterns
  ...
  samplesize_hist = N_hist, #Sample size of historical trial
  N_iter = No_iter,          # Number of iterations
  scenario_name = paste0(c('scenario1.3', paste0('iter', No_iter),"pattern_same"), collapse = '_')
)
...
run_simulation(
  ...
  samplesize_hist = N_hist, #Sample size of historical trial
  N_iter = No_iter,          # Number of iterations
  scenario_name = paste0(c('scenario1.3', paste0('iter', No_iter)), collapse = '_')
)
...
run_simulation(pattern_list = list( # Treatment patterns
  ...
  samplesize_hist = N_hist, #Sample size of historical trial
  N_iter = No_iter,          # Number of iterations
  scenario_name = paste0(c('scenario1.6', paste0('iter', No_iter),"pattern_same"), collapse = '_')
)
...
run_simulation(
  ...
  samplesize_hist = N_hist, #Sample size of historical trial
  N_iter = No_iter,          # Number of iterations
  scenario_name = paste0(c('scenario1.6', paste0('iter', No_iter)), collapse = '_')
)
